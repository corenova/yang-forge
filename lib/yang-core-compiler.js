// Generated by CoffeeScript 1.9.1
(function() {
  var MetaClass, YangCoreCompiler,
    extend = function(child, parent) { for (var key in parent) { if (hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; },
    hasProp = {}.hasOwnProperty,
    slice = [].slice;

  MetaClass = require('meta-class');

  YangCoreCompiler = (function(superClass) {
    extend(YangCoreCompiler, superClass);

    function YangCoreCompiler() {
      return YangCoreCompiler.__super__.constructor.apply(this, arguments);
    }

    YangCoreCompiler.set('schemadir', (require('path')).resolve(__dirname, '../schemas'));

    YangCoreCompiler.set({
      'yang/module': {
        resolver: function(arg, params) {
          return (function(superClass1) {
            extend(_Class, superClass1);

            function _Class() {
              return _Class.__super__.constructor.apply(this, arguments);
            }

            return _Class;

          })(this);
        },
        extension: '0..n',
        supplement: '0..n'
      },
      'yang/extension': {
        resolver: function(arg, params) {
          this.merge("yang/" + arg, params);
          return null;
        },
        argument: '0..1',
        description: '0..1',
        reference: '0..1',
        status: '0..1',
        sub: '0..n'
      },
      'yang/argument': {
        'yin-element': '0..1'
      },
      'yang/description': {
        'argument text': {
          'yin-element': true
        }
      },
      'yang/reference': {
        'argument text': {
          'yin-element': true
        }
      },
      'yang/status': {
        argument: 'value'
      },
      'yang/value': {
        argument: 'value'
      },
      'yang/yang-version': {
        argument: 'value'
      },
      'yang/yin-element': {
        argument: 'value'
      },
      'yang/sub': {
        argument: 'extension-name',
        resolver: function(arg, params) {
          return params != null ? params.value : void 0;
        },
        value: '0..1'
      },
      'yang/supplement': {
        argument: 'extension-name',
        resolver: function(arg, params) {
          this.merge("yang/" + arg, params);
          return null;
        },
        sub: '0..n'
      }
    });

    YangCoreCompiler.configure = function(func) {
      if (func != null) {
        if (typeof func.apply === "function") {
          func.apply(this);
        }
      }
      return this;
    };

    YangCoreCompiler.use = function() {
      var component, components, i, len, results1;
      components = 1 <= arguments.length ? slice.call(arguments, 0) : [];
      results1 = [];
      for (i = 0, len = components.length; i < len; i++) {
        component = components[i];
        if (component instanceof Function) {
          results1.push(this.merge(component));
        }
      }
      return results1;
    };

    YangCoreCompiler.readSchema = function(name) {
      var file;
      file = (require('path')).resolve(this.get('schemadir'), name);
      console.log("readSchema: " + file + "...");
      try {
        return (require('fs')).readFileSync(file, 'utf-8');
      } catch (_error) {
        console.log("WARN: unable to read from source " + arg);
        return void 0;
      }
    };

    YangCoreCompiler.parser = require('yang-parser');

    YangCoreCompiler.compile = function(schema) {
      var output, ref;
      if (schema == null) {
        return;
      }
      output = this.compileStatement(this.parser.parse(schema));
      if ((output != null ? (ref = output.value) != null ? typeof ref.get === "function" ? ref.get('yang') : void 0 : void 0 : void 0) === 'module') {
        output.value.merge(this.match(/.*\/.*/));
      }
      return output != null ? output.value : void 0;
    };

    YangCoreCompiler.compileStatement = function(statement) {
      var normalize, params, ref, ref1, ref2, results, stmt, target, value;
      if (!((statement != null) && statement instanceof Object)) {
        return;
      }
      if (!!statement.prf) {
        console.log("INFO: processing prefix keyword " + statement.prf + ":" + statement.kw);
        console.log(this.get("module/" + statement.prf));
        target = (ref = this.get("module/" + statement.prf)) != null ? typeof ref.get === "function" ? ref.get("yang/" + statement.kw) : void 0 : void 0;
      } else {
        target = this.get("yang/" + statement.kw);
      }
      normalize = function(statement) {
        return ([statement.prf, statement.kw].filter(function(e) {
          return (e != null) && !!e;
        })).join(':');
      };
      if (target == null) {
        console.log("WARN: unrecognized keyword extension '" + (normalize(statement)) + "', skipping...");
        return null;
      }
      if (statement.kw === 'module') {
        this.set("module/" + statement.arg, this);
      }
      results = (function() {
        var i, len, ref1, results1;
        ref1 = statement.substmts;
        results1 = [];
        for (i = 0, len = ref1.length; i < len; i++) {
          stmt = ref1[i];
          if ((function() {
            switch (false) {
              case !!(target.hasOwnProperty(stmt.kw)):
                console.log("WARN: " + statement.kw + " does not have sub-statement declared for " + stmt.kw);
                return false;
              default:
                return true;
            }
          })()) {
            results1.push(this.compileStatement(stmt));
          }
        }
        return results1;
      }).call(this);
      params = (results.filter(function(e) {
        return (e != null) && (e.value != null);
      })).reduce((function(a, b) {
        a[b.name] = b.value;
        return a;
      }), {});
      value = (function() {
        switch (false) {
          case !(target.resolver instanceof Function):
            return target.resolver.call(this, statement.arg, params, target);
          case !((Object.keys(params)).length > 0):
            return (function(superClass1) {
              extend(_Class, superClass1);

              function _Class() {
                return _Class.__super__.constructor.apply(this, arguments);
              }

              return _Class;

            })(MetaClass);
          default:
            return statement.arg;
        }
      }).call(this);
      if (value != null) {
        if (typeof value.set === "function") {
          value.set({
            yang: statement.kw
          });
        }
      }
      if (value != null) {
        if (typeof value.extend === "function") {
          value.extend(params);
        }
      }
      if ((target["export"] === true) || (target.meta === true)) {
        this.set(statement.kw + "/" + statement.arg, value);
      }
      if (target.meta === true) {
        return null;
      }
      switch (false) {
        case !(((ref1 = statement.substmts) != null ? ref1.length : void 0) > 0):
          return {
            name: (ref2 = statement.arg) != null ? ref2 : statement.kw,
            value: value
          };
        default:
          return {
            name: statement.kw,
            value: value
          };
      }
    };

    return YangCoreCompiler;

  })(MetaClass);

  module.exports = YangCoreCompiler;

}).call(this);
